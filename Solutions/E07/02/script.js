// Mithilfe Der Lösung geschafft

txt = `Plagiatsresolution und -maßnahmen 
	PDF export %filemtime%
...
function clearSonderzeichen(inp) {
	output = "";
	for (var i = 0; i < inp.length; i++) {
		c = inp.charAt(i).charCodeAt(0);
...
arr_split = txt.split(/\s/);
arr_filter = arr_split.filter(word => validWord(word));
arr_count = arr_filter.reduce((count, word) => {
                    if (!count[word]) 
						count[word] = 0;
                    count[word] += 1;
                    return count;
                }, {});
arr_sorted = Object.entries(arr_count).sort(([a, first], [, second]) => second - first);
arr = arr_sorted.slice(0, 3).map(lst => lst[0]);
